...
@section('content')
    <section class="content">
        <div class="container-fluid">
            ...
            <form role="form" action="/rsonline/pasienterlapor/patchkomorbid/{{ $data->lapKomorbidId }}" method="post">
                {{ csrf_field() }}
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Edit Laporan Komorbid Pasien</h3>
                    </div>
                    <!-- /.box-header -->
                                    ...
                                    <select name="komorbid" class="form-control select2" required>
                                        @foreach ($komorbid as $komorbid)
                                            <option value="{{ $komorbid->id }}-{{ $komorbid->nama }}"
                                                {{ $komorbid->id == $data->komorbidId ? 'selected' : '' }}>
                                                {{ $komorbid->nama }}
                                            </option>
                                        @endforeach
                                    </select>
