...
@section('content')
    <section class="content">
                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <form action="{{ route('berkasrm.bookingIndex') }}" method="GET">
                                ...
                                <table class="table-bordered table-sm table-hover" style="width: 100%;" id="example">
                                    ...
                                    <tbody>
                                        @forelse($data as $book)
                                            <tr>
                                                <td class="text-center"><input type="checkbox" name="no_pasien[]" value="{{ $book->no_rkm_medis }}"></td>
                                                <td>{{ $book->nm_pasien }}</td>
                                                <td>{{ $book->no_rkm_medis }}</td>
                                                <td>{{ $book->no_reg }}</td>
                                                <td>{{ $book->nm_poli }}</td>
                                                <td>{{ $book->nm_dokter }}</td>
                                                <td>{{ $book->png_jawab }}</td>
                                                <td>{{ $book->tanggal_booking }}</td>
                                                <td>{{ $book->jam_booking }}</td>
                                                <td>{{ \Carbon\Carbon::parse($book->tanggal_periksa)->format('Y-m-d') }}</td>
                                                <td>{{ $book->status }}</td>
                                                <td class="text-center"><a href="#" id="kirimPesan" data-id="{{ Crypt::encrypt($book->no_rkm_medis) }}" data-toggle="tooltip" data-placement="bottom" title="Kirim Pesan"><i class="fab fa-whatsapp text-success text-bold btn "></i></a></td>
                    ...
                    <div class="modal fade" id="templateModal" tabindex="-1" aria-labelledby="templateModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <form method="POST" action="{{ route('berkasrm.bookingKirimBlast') }}">
                            @csrf
        ...
        <div class="modal fade" id="kirim_modal">
            <div class="modal-dialog">
                <form action="{{ route('berkasrm.bookingKirimPesan') }}" method="POST">
                    @csrf
                    <div class="modal-content ">
                        <div class="modal-header">
