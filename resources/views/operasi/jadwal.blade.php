            ...
            <div class="row">
                @php
                    if (!empty(Request::get('tanggalMulai')) && !empty(Request::get('tanggalSelesai'))) {
                        ...
                        $tanggalMulai = \Carbon\Carbon::now()
                            ->yesterday()
                            ->format('Y-m-d');
                        ...
                        <div class="card-header">
                            ...
                            <div class="form-group row">
                                <div class="col-sm-9 mt-2">
                                    <label>Jadwal Pasien Booking Operasi</label>
                                </div>
                                <div class="col-sm-3">
                                    <form action="/operasi/jadwal" method="GET">
                                ...
                                <table class="table table-bordered table-hover table-sm" id="example2">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="align-middle">Ruang Operasi</th>
                                        ...
                                        @foreach ($data as $dataOperasi)
                                            <tr>
                                                ...
                                                @php
                                                    ...
                                                    // tahun
                                                    $umur = $today->diff($dataOperasi->tgl_lahir)->y;
                                                @endphp
                                                <td class="align-middle">{{ $dataOperasi->no_rawat }}</td>
                                                <td class="align-middle">{{ $dataOperasi->nm_pasien }}</td>
                                                <td class="align-middle">{{ $umur }} Th</td>
                                                <td class="align-middle">{{ $dataOperasi->jk }}</td>
                                                <td class="align-middle">{{ $dataOperasi->tanggal }}</td>
                                                <td class="align-middle">{{ $dataOperasi->jam_mulai }}</td>
                                                <td class="align-middle">{{ $dataOperasi->jam_selesai }}</td>
                                                <td class="align-middle">
                                                    @if ($dataOperasi->status == 'Selesai')
                                                        <span
                                                            class="right badge badge-success">{{ $dataOperasi->status }}</span>
                                                    @elseif ($dataOperasi->status == 'Batal')
                                                        <span
                                                            class="right badge badge-danger">{{ $dataOperasi->status }}</span>
                                                    @else
                                                        <span
                                                            class="right badge badge-warning">{{ $dataOperasi->status }}</span>
                                                    @endif
                                                </td>

                                                <td class="align-middle">{{ $dataOperasi->nm_perawatan }}</td>
                                                <td class="align-middle">{{ $dataOperasi->nm_dokter }}</td>
                                                <td class="align-middle">{{ $dataOperasi->nm_ruang_ok }}</td>
                                                {{-- <td class="align-middle">{{ $dataOperasi->tanggal }}</td>
                                                <td>

                                                    <a href="/operasi/{{ Crypt::encrypt($dataOperasi->id_booking) }}/booking"
                                                        class="btn btn-success btn-sm {{ $dataOperasi->status == 'Booking' ? 'disabled' : '' }}"
