                        ...
                        <div class="card-body">
                            {{-- <div style="overflow-x:auto;"> --}}
                                <table class="table table-bordered table-hover table-sm" id="example2">
                                    <thead>
                                        <tr>
                                            <th class="align-middle">Kode Template</th>
                                    ...
                                    <tbody>
                                        @if ($data)
                                            @foreach ($data as $list)
                                                <tr>
                                                    <td>{{ $list->id_template }}</td>
                                                    <td>{{ $list->Pemeriksaan }}</td>
                                                        ...
                                                        <div class="col text-center">
                                                            <div class="btn-group">
                                                                <button class="btn btn-sm btn-primary btn-edit"
                                                                    title="Pilih"
                                                                    data-id="{{ Crypt::encrypt($list->id_template) }}">
                                                                    <i class="fas fa-hand-pointer"></i>
                                                                </button>
                                                                {{-- <a href="{{ route('satuSehat.mapingTemplateLabEdit', Crypt::encrypt($list->id_template)) }}" class="btn btn-sm btn-primary btn-edit"
                                                                    title="Pilih"
                                                                    >
                                                                    <i class="fas fa-hand-pointer"></i>
                                                                </a> --}}
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="id_template" class="form-control" readonly>
                                    </div>

                                    <div class="form-group">
                                        <label>Nama Template</label>
                                        <input type="text" id="pemeriksaan" class="form-control" readonly>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" id="nama_pemeriksaan" class="form-control" readonly>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <hr>
...
@section('get')
    ...
    <script>
        $(document).on('click', '.btn-edit', function() {
            ...
            $.ajax({
                url: '/satusehat/mapingtemplatelab/' + id + '/edit',
                type: 'GET',
                success: function(res) {
                    console.log(res);

                    $('#id_template').val(res.id_template);
                    $('#pemeriksaan').val(res.Pemeriksaan);
                    $('#kelas').val(res.kelas);
                    $('#nama_pemeriksaan').val(res.nm_perawatan);
    ...
    <script>
        $('#formEditPegawai').on('submit', function(e) {
            ...
            let idTemplate = $('#id_template').val();
            ...
            $.ajax({
                url: '{{ route('satuSehat.mapingTemplateLabUpdate') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    _method: 'PUT',
